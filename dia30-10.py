...
def abrir_janela_movimentacao(tipo):
    ...
    btn_confirmar = Button(janela_m, text=tipo,
           command=lambda: [
               atualizar_estoque(
                   produto_id,
...
def abrir_janela_fechamento_caixa():
    ...
    janela_f.configure(bg='#E8E8E8')  # Cinza suave

    # Estilo claro específico para as tabelas desta janela
    style_light = ttk.Style()
    style_light.configure('Light.Treeview',
                          background='#F5F5F5',
                          fieldbackground='#F5F5F5',
                          foreground='black',
                          rowheight=26)
    style_light.map('Light.Treeview',
                    background=[('selected', LIGHT_BUTTON_ACTIVE)],
                    foreground=[('selected', LIGHT_TEXT)])
    style_light.configure('Light.Treeview.Heading',
                          background=COLOR_GOLD,
                          foreground='black',
                          font=("Segoe UI", 11, "bold"))
    
    # Melhorando a aparência dos frames dentro da janela
    frame_style = {'bg': '#E8E8E8', 'padx': 10, 'pady': 10}
    ...
    Label(frame_filtros_rapidos, text="Filtros Rápidos:", bg='#E8E8E8', font=("Arial", 10, "bold")).pack(side=LEFT, padx=8)
    
    # Cor de botão mais harmônica com tema claro
    BUTTON_LIGHT_BG = '#F2D27A'
    BUTTON_LIGHT_FG = 'black'
    
    btn_hoje = Button(frame_filtros_rapidos, text="Hoje", bg=BUTTON_LIGHT_BG, fg=BUTTON_LIGHT_FG, command=aplicar_filtro_hoje)
    btn_hoje.pack(side=LEFT, padx=6, pady=4)
    
    btn_ontem = Button(frame_filtros_rapidos, text="Ontem", bg=BUTTON_LIGHT_BG, fg=BUTTON_LIGHT_FG, command=aplicar_filtro_ontem)
    btn_ontem.pack(side=LEFT, padx=6, pady=4)
    
    btn_mes_atual = Button(frame_filtros_rapidos, text="Mês Atual", bg=BUTTON_LIGHT_BG, fg=BUTTON_LIGHT_FG, command=aplicar_filtro_mes_atual)
    btn_mes_atual.pack(side=LEFT, padx=6, pady=4)
    
    btn_mes_anterior = Button(frame_filtros_rapidos, text="Mês Anterior", bg=BUTTON_LIGHT_BG, fg=BUTTON_LIGHT_FG, command=aplicar_filtro_mes_anterior)
    btn_mes_anterior.pack(side=LEFT, padx=6, pady=4)
    
    btn_30_dias = Button(frame_filtros_rapidos, text="Últimos 30 dias", bg=BUTTON_LIGHT_BG, fg=BUTTON_LIGHT_FG, command=aplicar_filtro_ultimos_30_dias)
    ...
    Label(frame_filtros, text="Período:", bg='#E8E8E8').pack(side=LEFT, padx=5)
    Label(frame_filtros, text="De:", bg='#E8E8E8').pack(side=LEFT)
    e_ini = Entry(frame_filtros, width=10)
    e_ini.pack(side=LEFT, padx=2)
    e_ini.insert(0, date.today().strftime("%Y-%m-%d"))
    
    Label(frame_filtros, text="Até:", bg='#E8E8E8').pack(side=LEFT)
    ...
    def carregar():
        ...
        frame_produtos = LabelFrame(frame_resultados, text="Movimentação de Produtos", bg='#B6B6B6', font=("Arial", 10, "bold"))
        ...
        tree_produtos = ttk.Treeview(frame_produtos, columns=cols_produtos, show='headings', height=8, style='Light.Treeview')
        ...
        frame_servicos = LabelFrame(frame_resultados, text="Serviços Realizados", bg='#B6B6B6', font=("Arial", 10, "bold"))
        ...
        tree_servicos = ttk.Treeview(frame_servicos, columns=cols_servicos, show='headings', height=8, style='Light.Treeview')
        ...
        frame_resumo = LabelFrame(frame_resultados, text="RESUMO DO PERÍODO", bg='#B6B6B6', font=("Arial", 10, "bold"))
        frame_resumo.pack(fill="both", padx=5, pady=5)
        
        # Adiciona informações do resumo
        Label(frame_resumo, text=f"Total em Serviços: R$ {total_servicos:.2f}", bg='#B6B6B6', font=("Arial", 10)).pack(anchor='w', padx=10, pady=2)
